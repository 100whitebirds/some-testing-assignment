...
const FormEdit: FC<{id: string}> = ({ id }) => {
  ...
  const selectHandler = (item: IContact['firstImpression']) => {
    Store.formData.firstImpression = item
    setDropdownOpen(false)
  }

const submitHandler = async() => {
    const success = await Store.editContact()
    if (success) {
      Store.resetForm()
  ...
  const deleteHandler = async() => {
    const success = await Store.deleteContact()
    if (success) {
      Store.resetForm()
      history.push(RouteNames.HOME)
    }
            ...
            { dropdownOpen &&
              <DropDownList>
                { Object.values(Impressions).map(item =>
                  <Item onClick={() => selectHandler(item as IContact['firstImpression'])}>
                    { item }
                  </Item>
                )}
              </DropDownList>
...
export const ContactFormEdit = observer(FormEdit)
